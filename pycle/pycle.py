...
@click.option('-h', '--host', required=True, help=u'IP地址')
@click.option('-p', '--port', required=True, help=u'监听端口')
@click.option('-s', '--service-name', required=True, help=u'服务名')
@click.option('-u', '--user', required=True, help=u'用户名')
@click.option('-P', '--passwd', required=True, help=u'密码')
@click.pass_context
def pycle(ctx, host, port, service_name, user, passwd):
    '''
    A CLI utility to check database metrics\n
    example: \n
    pycle -h 127.0.0.1 -p 1521 -s orcl -u pycle -P pycle get-archivelog-stats
    '''
